...
(function () {
    ...
    a.prototype.draw = function () {
        ...
        this.bounds = k;
        var j = this._map.pointToOverlayPixel(k.getNorthEast()), p = this._map.pointToOverlayPixel(k.getSouthWest()),
            c = j.y, i = p.x, l = p.y - j.y, o = j.x - p.x;
        ...
        if (this.latlngs.length > 0) {
            ...
            while (n--) {
                ...
                var g = this._map.pointToOverlayPixel(f), i = this._map.pointToOverlayPixel(k.getSouthWest()).x,
                    c = this._map.pointToOverlayPixel(k.getNorthEast()).y, e = new BMap.Pixel(g.x - i, g.y - c);
    ...
    a.prototype.pixelTransform = function (f) {
        var c = this.heatmap.width, e = this.heatmap.height;
        while (f.x < 0) {
            f.x += c
        }
        while (f.x > c) {
    ...
    a.prototype.setDataSet = function (j) {
        ...
        while (k--) {
            ...
            var g = this._map.pointToOverlayPixel(f), h = this._map.pointToOverlayPixel(i.getSouthWest()).x,
                c = this._map.pointToOverlayPixel(i.getNorthEast()).y, e = new BMap.Pixel(g.x - h, g.y - c);
    ...
    a.prototype.addDataPoint = function (e, g, f) {
        ...
        var h = new BMap.Point(e, g), c = this.pixelTransform(this._map.pointToOverlayPixel(h));
